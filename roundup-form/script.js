    ...
    nextButtons.forEach(button => {
        button.addEventListener('click', () => {
            if (currentStep === 1) {
                ...
                [firstNameField, lastNameField, emailField, phoneField].forEach(field => {
                ...
                const roundupCancer = document.getElementById('roundup-cancer');
                const roundupExposed = document.getElementById('roundup-exposed');
                const roundupYear = document.getElementById('roundup-year');
                const roundupAttorney = document.getElementById('roundup-attorney');
                
                let emptyFields = [];
                let isValid = true;
                
                if (!roundupCancer.value) {
                    roundupCancer.classList.add('error');
                    document.querySelector('label[for="roundup-cancer"]').classList.add('error');
                    emptyFields.push('Cancer Type');
                    isValid = false;
                }
                
                if (!roundupExposed.value) {
                    roundupExposed.classList.add('error');
                    document.querySelector('label[for="roundup-exposed"]').classList.add('error');
                    emptyFields.push('Exposure');
                    isValid = false;
                }
                
                if (!roundupYear.value) {
                    roundupYear.classList.add('error');
                    document.querySelector('label[for="roundup-year"]').classList.add('error');
                    emptyFields.push('Diagnosis Year');
                    isValid = false;
                }
                
                if (!roundupAttorney.value) {
                    roundupAttorney.classList.add('error');
                    document.querySelector('label[for="roundup-attorney"]').classList.add('error');
                    emptyFields.push('Attorney Representation');
                    isValid = false;
                }
                ...
                if (!isValid) {
                    ...
                    const firstErrorField = document.querySelector('.form-step[data-step="2"] .error');
                    if (firstErrorField) firstErrorField.focus();
                    
                    return;
                }
                
                [roundupCancer, roundupExposed, roundupYear, roundupAttorney].forEach(field => {
                    field.addEventListener('input', function() {
                        if (this.value.trim()) {
                            this.classList.remove('error');
                            const label = document.querySelector(`label[for="${this.id}"]`);
